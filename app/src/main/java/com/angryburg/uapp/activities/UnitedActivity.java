...
public interface UnitedActivity {
     ...
     * Session variables are activity-wide, and are not preserved through finish()es
     * or app restarts
    ...
    /**
     * finishes() the activity, optionally reloading the page that launched this
     * activity.
     * For example, the nanocamo "app" changes the theme, and index.html only checks
     * the theme
     * on page load. So when you change theme in nanocamo, it needs to notify
     * index.html and will set
     * refresh to true.
     * 
     * @param refresh true if the page that opened this activity with launchHTML
     *                should be refreshed
     */
    void closeWindow(boolean refresh);

    /**
     * An interface can't extend or implement Activity, so to call methods like
     * setTitle on it a UnitedActivity
